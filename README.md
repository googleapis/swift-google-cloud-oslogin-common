...
```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-oslogin-common.git --from 0.3.0
```

Then add `GoogleCloudOSLoginCommon` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudOSLoginCommon <target-name> --package swift-google-cloud-oslogin-common
```
